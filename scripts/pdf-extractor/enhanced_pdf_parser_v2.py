...
class RegexPatterns:
    ...
    # Question detection - Multiple patterns for different exam boards
    # Cambridge/OCR: "1 What is..." or "Q1: What is..."
    QUESTION_NUMBER = re.compile(r'^\s*(\d{1,2})\s+([A-Z])', re.MULTILINE)
    QUESTION_PREFIX = re.compile(r'^Q(\d{1,2}):\s*', re.IGNORECASE)
    # AQA format: "0 1" or "0 2" (space-separated two-digit)
    QUESTION_AQA = re.compile(r'^\s*0\s*(\d{1,2})\s+', re.MULTILINE)
    # Edexcel format: questions with (Total for Question X = Y marks)
...
def normalize_question_markers_optimized(text: str) -> str:
    ...
    Focuses on cleaning text and normalizing basic markers.
    Let GPT handle complex question structure detection.
    """

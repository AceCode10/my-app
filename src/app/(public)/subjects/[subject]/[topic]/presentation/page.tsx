    ...
    async function load() {
      try {
        const { data: subjectRow } = await supabase
          .from('subjects').select('id').eq('slug', subjectSlug).single();
        if (!subjectRow) throw new Error('Subject not found');

        const { data: topicRow } = await supabase
          .from('topics').select('id')
          .eq('subject_id', subjectRow.id).eq('slug', topicSlug).single();
  ...
  if (error || !presentationUrl) {
    return (
      <div className="flex flex-col items-center justify-center w-full min-h-screen bg-gray-950 text-white gap-4 p-8">
        ...
        <p className="text-xl font-semibold text-center">{error || 'No presentation available'}</p>
        <Link href={backHref}>
          <Button variant="outline" className="mt-4">Back to topic</Button>
        </Link>
      </div>
    );
  }

  return <HtmlDeckPresenter url={presentationUrl} title={noteTitle} backHref={backHref} />;
}

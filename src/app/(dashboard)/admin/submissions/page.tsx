    ...
    try {
      ...
      const enrichedSubmissions = await Promise.all(
        (attemptsData || []).map(async (attempt) => {
          ...
          // Fetch paper with subject
          const { data: paperData } = await supabase
            .from('past_papers')
            .select('id, title, year, session, total_marks, subject_id')
            .eq('id', attempt.paper_id)
            .single();

          // Fetch subject
          let subjectData = null;
          if (paperData?.subject_id) {
            const { data } = await supabase
              .from('subjects')
              .select('id, name')
              .eq('id', paperData.subject_id)
              .single();
            subjectData = data;
          }
      ...
      let filtered = enrichedSubmissions.filter(s => s.user && s.paper);
      if (selectedSubject !== 'all') {
        filtered = filtered.filter(s => s.paper?.subject_id === selectedSubject);
      }

      setSubmissions(filtered as Submission[]);
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-3xl font-bold flex items-center gap-3">
          ...
          Self-Practice Submissions
        </h1>
        <p className="text-muted-foreground mt-1">
          Review and provide feedback on student self-practice submissions
        </p>
      </div>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground">
                {submissions.length === 0 
                  ? "No self-practice submissions to review"

...
import type { Note, Subject as SubjectType } from '@/types';
...
const noteSchema = z.object({
    ...
    // If visibility is not 'draft', then renderedHtml is required.
    if (data.visibility !== 'draft') {
        return !!data.renderedHtml && data.renderedHtml.length > 0;
    }
    return true;
}, {
    message: "Rendered HTML must be provided for non-draft notes. Use the AI generator or manually add it.",
    path: ["renderedHtml"],
});
...
const NoteEditorPage = () => {
    ...
    const selectedSubject = useMemo(() => subjects.find(s => s.slug === subjectIdValue), [subjects, subjectIdValue]);
    ...
    useEffect(() => {
        // When subjectIdValue changes, reset the topicId field.
        if (form.getValues('topicId')) {
            form.resetField('topicId', { defaultValue: '' });
        }
    }, [subjectIdValue, form]);
    
    const backPath = pathname.includes('/admin/') ? '/admin/dashboard/notes' : '/teacher/dashboard/notes';
    ...
    useEffect(() => {
        ...
        if (!firestore || typeof noteId !== 'string' || !noteId) {
            toast({
              variant: 'destructive',
              title: 'Invalid Note ID',
              description: 'The note ID is missing or invalid. Cannot load data.',
            });
            ...
            try {
                ...
                if (noteSnap.exists()) {
                    ...
                    form.reset({
                        ...
                        visibility: noteData.visibility || 'draft',
                        contentRaw: noteData.contentRaw || '',
                        renderedHtml: noteData.renderedHtml || '',
                        contentFormat: noteData.contentFormat || 'richTextJson',
                    });
                } else {
        ...
        fetchNote();
    }, [noteId, isNewNote, firestore, router, toast, backPath, form]);
    ...
    const handleAiGenerate = async () => {
        ...
        setIsLoading(true);
        toast({ title: 'AI Generating...', description: `Creating note content for ${topicIdValue}. This might take a moment.` });
        try {
            ...
            toast({ title: 'Content Generated!', description: 'The AI-generated content has been added to the rendered HTML field.' });
        } catch (error) {
    ...
    const onSubmit = async (data: NoteFormData) => {
        ...
        try {
            const noteData: Partial<Omit<Note, 'id'>> = {
                title: data.title,
            ...
            if (isNewNote) {
                const newDocRef = await addDoc(collection(firestore, 'notes'), {
                    ...noteData,
                    createdAt: serverTimestamp(),
                    publishedAt: data.visibility === 'public' ? serverTimestamp() : null,
                    version: 1,
                });
                ...
                await setDoc(noteRef, {
                    ...noteData,
                    version: increment(1),
                    ...(data.visibility === 'public' && !isNewNote && { publishedAt: serverTimestamp() }),
                }, { merge: true });
            ...
            <form onSubmit={form.handleSubmit(onSubmit)}>
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Fill out the details for the note. Raw content can be in Markdown, which will be rendered to HTML.</CardDescription>
                    </CardHeader>
                         ...
                         <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl><Input placeholder="A brief summary of the note's content" {...field} /></FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
                        />
                            ...
                            />
                             <FormField
                                ...
                                <FormItem>
                                    ...
                                    <Select onValueChange={field.onChange} value={field.value} defaultValue={field.value} disabled={!subjectIdValue || isLoadingTopics}>
                                        <FormControl>
                                            <SelectTrigger><SelectValue placeholder={!subjectIdValue ? "Select a subject first" : (isLoadingTopics ? 'Loading topics...' : 'Select a topic')} /></SelectTrigger>
                                        </FormControl>
                                        <SelectContent>
                                             {topics?.map(topic => (
                         ...
                         <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl><Textarea placeholder="<h1>Title</h1><p>Content goes here...</p>" {...field} rows={15} /></FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
                        />

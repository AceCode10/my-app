...
import { ChevronRight, FileText, Download, CheckCircle, FileArchive, FlaskConical, TestTube } from 'lucide-react';
        ...
        <div>
            ...
            <div className="mb-8 flex items-center space-x-4">
                <div className="bg-muted p-4 rounded-lg">
                    <subjectData.icon className="w-10 h-10 text-primary" />
                </div>
                <div>
                    <h1 className="text-4xl font-extrabold text-foreground">{subjectData.name}</h1>
                    <p className="text-muted-foreground mt-1">Syllabus Code: {subjectData.code}</p>
                </div>
            </div>
             ...
             <Tabs defaultValue="notes" className="w-full">
                <TabsList className="grid w-full grid-cols-4">
                    ...
                    {subjectData.practicals && <TabsTrigger value="practicals">🧪 Practicals</TabsTrigger>}
                </TabsList>
                <TabsContent value="notes" className="mt-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {subjectData.topics?.map(topic => (
                           <Link href={`/dashboard/subjects/${params.subject}/${topic.name.toLowerCase().replace(/ /g, '-')}`} key={topic.name}>
                ...
                <TabsContent value="past-papers" className="mt-6">
                 ...
                 <TabsContent value="quizzes" className="mt-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {subjectData.topics?.map(topic => (
                           <Link href={`/dashboard/subjects/${params.subject}/${topic.name.toLowerCase().replace(/ /g, '-')}/quiz`} key={topic.name}>
                             <div className="bg-background p-6 rounded-2xl shadow-sm border hover:border-primary hover-shadow-lg transition-all duration-200 h-full flex flex-col">
                                <h4 className="font-bold text-lg text-foreground">{topic.name}</h4>
                                <p className="text-sm text-muted-foreground mt-2 flex-grow">{topic.description}</p>
                 ...
                 <TabsContent value="practicals" className="mt-6">
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {practicalResources.map(resource => (
                            <div key={resource.name} className="bg-background p-6 rounded-2xl shadow-sm border flex items-start space-x-4">
                               <div className="bg-muted p-3 rounded-lg">
                                 <resource.icon className="w-6 h-6 text-primary" />
                               </div>
                               <div>
                                 <h4 className="font-bold text-lg text-foreground">{resource.name}</h4>

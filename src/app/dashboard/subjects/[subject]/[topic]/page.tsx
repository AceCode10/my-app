...
import { allSubjects } from '@/lib/subjects.tsx';
...
export default function TopicPage({ params }: { params: { subject: string, topic: string }}) {
    ...
    const notesQuery = useMemoFirebase(() => {
        ...
        return query(
            ...
            // In a real app, we might check for premium/registered status here
    ...
    const resources = [
        { name: "Revision Notes", href: `/dashboard/subjects/${subjectSlug}/${topicSlug}/notes`, icon: BookOpen, enabled: true },
        { name: "Flashcards", href: `/dashboard/subjects/${subjectSlug}/${topicSlug}/flashcards`, icon: Layers, enabled: true },
        { name: "Topical Quiz", href: `/dashboard/subjects/${subjectSlug}/${topicSlug}/quiz`, icon: FileText, enabled: true },
    ];

    return (
        <div>
            <div className="flex items-center text-sm text-muted-foreground mb-4">
                <Link href="/dashboard/subjects" className="hover:text-primary">My Subjects</Link>
                ...
                <aside className="lg:col-span-1 bg-card p-4 rounded-2xl shadow-sm border sticky top-24">
                    ...
                    <nav className="flex flex-col space-y-1">
                        {subjectData.topics?.map(t => {
                            ...
                            const href = `/dashboard/subjects/${subjectSlug}/${currentTopicSlug}`;
                        ...
                        <CardContent>
                            {isLoadingNotes ? (
                                ...
                                <div className="space-y-2">
                                    {notes.map(note => (
                                        <Link key={note.id} href={`/dashboard/subjects/${subjectSlug}/${topicSlug}/notes`}>

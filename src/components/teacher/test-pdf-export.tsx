...
const styles = StyleSheet.create({
  ...
  // Question image style for image-heavy subjects - maintain aspect ratio
  questionImage: {
    maxWidth: '100%',
    maxHeight: 350,
    marginBottom: 12,
    ...
    <Document>
      ...
      <Page size="A4" style={styles.coverPage}>
        ...
        <Text style={styles.footer} fixed>
          This document was generated by RevisionPlus
        </Text>
      </Page>
      ...
      <Page size="A4" style={styles.page}>
        {test.sections && Array.isArray(test.sections) && test.sections.map((section, sectionIndex) => (
          <View key={sectionIndex}>
            {section.questions.map((tq, qIndex) => {
              ...
              return (
                <View key={tq.questionId} style={{ marginBottom: 20 }} wrap={false}>
                  {/* Question Row: Number | Content | (marks on last line) */}
                  <View style={styles.questionRow}>
                    <Text style={styles.questionNumberCol}>{questionNumber}</Text>
                    <View style={styles.questionContentCol}>
                      ...
                      {hasImageUrl && (
                        <Image src={(question.question_image_url || question.image_url)!} style={styles.questionImage} />
                      )}
                      ...
                      {/* Answer Lines - Cambridge style solid lines */}
                      {lineCount > 0 && (
                      ...
                      {/* For MCQ/TF - show marks after options */}
                      {(question.question_type === 'mcq' || question.question_type === 'multiple_choice' || question.question_type === 'Multiple Choice' || question.question_type === 'tf') && (
        ...
        {/* Footer */}
        <Text style={styles.footer} fixed>
          © RevisionPlus {new Date().getFullYear()}
        </Text>
        <Text
      ...
      if (test.sections && test.sections.length > 0 && test.sections[0].questions && test.sections[0].questions.length > 0) {
        ...
        if (aqData && aqData.length > 0) {
          ...
          const questionsWithData = aqData.map((aq: any, index: number) => ({
            ...
            marks: aq.custom_marks || questionsData?.find((q: any) => q.id === aq.question_id)?.marks || 1,
            order: index + 1,
            question: questionsData?.find((q: any) => q.id === aq.question_id) || null
          }));
